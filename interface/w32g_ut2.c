...
//      return -1;
    ...
    SendMessage(hwndFontChange, WM_SETFONT, (WPARAM) hFont, MAKELPARAM(TRUE,0));
    if (fontname != NULL) strcpy(fontname, lf.lfFaceName);
    if (fontname != NULL) fontname[LF_FACESIZE - 1] = 0;
...
int DlgChooseFont(HWND hwnd, char *fontName, int *fontHeight, int *fontWidth)
{
    ...
    ZeroMemory(&lf, sizeof(LOGFONTA));
    if (fontHeight) lf.lfHeight = *fontHeight;
    if (fontWidth) lf.lfWidth = *fontWidth;
    if (fontName) strcpy(lf.lfFaceName, fontName);
...
int INISaveMainWnd(void)
{
    ...
    if ( PosSizeSave ) {
        ...
        if ( MainWndInfo.PosX >= 0 )
            WritePrivateProfileStringA(section,"PosX",buffer,inifile);
        sprintf(buffer,"%d",MainWndInfo.PosY);
        if ( MainWndInfo.PosY >= 0 )
            WritePrivateProfileStringA(section,"PosY",buffer,inifile);
        sprintf(buffer,"%d",MainWndInfo.Width);
        WritePrivateProfileStringA(section,"Width",buffer,inifile);
        sprintf(buffer,"%d",MainWndInfo.Height);
        WritePrivateProfileStringA(section,"Height",buffer,inifile);
    }
    sprintf(buffer,"%d",MainWndInfo.CanvasMode);
    WritePrivateProfileStringA(section,"CanvasMode",buffer,inifile);
...
int INILoadMainWnd(void)
{
    ...
    if (num == -1) num = DEFAULT_CANVAS_MODE;
...
int INISaveListWnd(void)
{
    ...
    WritePrivateProfileStringA(section,"fontNameEN",ListWndInfo.fontNameEN,inifile);
    WritePrivateProfileStringA(section,"fontNameJA",ListWndInfo.fontNameJA,inifile);
    sprintf(buffer,"%d",ListWndInfo.fontWidth);
    WritePrivateProfileStringA(section,"fontWidth",buffer,inifile);
    sprintf(buffer,"%d",ListWndInfo.fontHeight);
    WritePrivateProfileStringA(section,"fontHeight",buffer,inifile);
    sprintf(buffer,"%d",ListWndInfo.fontFlags);
    WritePrivateProfileStringA(section,"fontFlags",buffer,inifile);
    // ListName
    for (i = 0; i < playlist_max; i++) {
        snprintf(key, sizeof(key), "ListName%d", i);
        WritePrivateProfileStringA(section,key,ListWndInfo.ListName[i],inifile);
    }
    // columWidth
    for (i = 0; i < LISTWND_COLUM; i++) {
        snprintf(key, sizeof(key), "ColumWidth%d", i);
        sprintf(buffer,"%d",ListWndInfo.columWidth[i]);
        WritePrivateProfileStringA(section,key,buffer,inifile);
    }
    //
    WritePrivateProfileStringA(NULL,NULL,NULL,inifile);              // Write Flush
    return 0;
...
int INILoadListWnd(void)
{
    ...
    char *inifile = TIMIDITY_WINDOW_INI_FILE;
    int num;
    char buffer[256]; // >= LF_FULLFACESIZE(64)
    int i;
    char key[30] = "", name[30] = "";
    ...
    GetPrivateProfileStringA(section,"fontNameEN","",buffer,LF_FULLFACESIZE + 1,inifile);
    if (buffer[0]!=0) strcpy(ListWndInfo.fontNameEN,buffer);
    GetPrivateProfileStringA(section,"fontNameJA","",buffer,LF_FULLFACESIZE + 1,inifile);
    ...
    for (i = 0; i < PLAYLIST_MAX; i++) {
        ...
        GetPrivateProfileStringA(section,key,name,buffer,ARRAY_SIZE(buffer),inifile);
        if (buffer[0]!=0) strcpy(ListWndInfo.ListName[i],buffer);
    }
    // columWidth
    for (i = 0; i < LISTWND_COLUM; i++) {
        snprintf(key, sizeof(key), "ColumWidth%d", i);
        num = GetPrivateProfileIntA(section,key, def_columWidth[i],inifile);
...
int INISaveDocWnd(void)
{
    ...
    WritePrivateProfileStringA(section,"fontNameEN",DocWndInfo.fontNameEN,inifile);
    WritePrivateProfileStringA(section,"fontNameJA",DocWndInfo.fontNameJA,inifile);
...
int INILoadDocWnd(void)
{
    ...
    int num;
    char buffer[256]; // >= LF_FULLFACESIZE(64)
    num = GetPrivateProfileIntA(section,"PosX",-1,inifile);
    DocWndInfo.PosX = num;
    num = GetPrivateProfileIntA(section,"PosY",-1,inifile);
    DocWndInfo.PosY = num;
    num = GetPrivateProfileIntA(section,"Width",-1,inifile);
    if (num!=-1) DocWndInfo.Width = num;
    num = GetPrivateProfileIntA(section,"Height",-1,inifile);
    ...
    num = GetPrivateProfileIntA(section,"fontWidth",-1,inifile);
    if (num!=-1) DocWndInfo.fontWidth = num;
    num = GetPrivateProfileIntA(section,"fontHeight",-1,inifile);
    if (num!=-1) DocWndInfo.fontHeight = num;
    num = GetPrivateProfileIntA(section,"fontFlags",-1,inifile);
...
int SendFilesToOldTiMidity(int nfiles, char **files)
{
    ...
    for (i=0;i<nfiles;i++) {
        ...
        char *p;
//      if (url_check_type(files[i])==-1 && GetFullPathNameA(files[i],1000,filepath,&p) !=0) {
        if (isURLFile(files[i])==FALSE && GetFullPathNameA(files[i],FILEPATH_MAX-1,filepath,&p) !=0) {
