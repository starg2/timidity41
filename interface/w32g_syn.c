...
static INT_PTR CALLBACK SynWinProc(HWND hwnd, UINT uMess, WPARAM wParam, LPARAM lParam);
                                ...
                                LPSTR lpCmdLine, int nCmdShow)
{
    ...
    {
        if (!stricmp(w32g_syn.argv[i], "/QUIET"))
        {
            quietInstaller = TRUE;
        }
        else if (!stricmp(w32g_syn.argv[i], "/VERBOSE"))
        {
            quietInstaller = FALSE;
        }
        else if (!stricmp(w32g_syn.argv[i], "/INSTALL"))
        {
            InstallService();
...
//  wrdt = wrdt_list[0];
    ...
    if (RestartTimidity) {
        ...
        if (CreateProcessA(path, lpCmdLine, NULL, NULL, TRUE, CREATE_DEFAULT_ERROR_MODE, NULL, NULL, &si, &pi) == FALSE)
            MessageBox(NULL, TEXT("Restart Error."), TEXT("TiMidity++ Synth Win32GUI"), MB_OK | MB_ICONEXCLAMATION);
    }
    return 0;
}
...
//  syn_ThreadPriority = THREAD_PRIORITY_NORMAL;
    ...
    if (w32g_syn_create_win()) {
        MessageBox(NULL, TEXT("Fatal Error"), TEXT("TWSYNG32 ERROR"), MB_OK);
        return -1;
    }
...
///r
                    ...
                    AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuReset, TEXT("各種システムリセット"));
                    AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuChange, TEXT("特定のシステムへ変更"));
                    AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuModule, TEXT("特定の音源モードへ変更"));

                    if (uMess != WM_LBUTTONDOWN) {
                        AppendMenu(hMenu, MF_SEPARATOR, 0, 0);
                        AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuProcessPriority, TEXT("プロセスプライオリティ設定"));
                        AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuSynPriority, TEXT("シンセスレッドプライオリティ設定"));
                        AppendMenu(hMenu, MF_SEPARATOR, 0, 0);
...
///r
                    ...
                    AppendMenu(hMenu, MF_SEPARATOR, 0, 0);
                    AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuReset, TEXT("Specific system reset"));
                    AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuChange, TEXT("Change Specific system"));
                    AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuModule, TEXT("Change Specific Module"));

                    if (uMess != WM_LBUTTONDOWN) {
                        AppendMenu(hMenu, MF_SEPARATOR, 0, 0);
                        AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuProcessPriority, TEXT("Change process priority"));
                        AppendMenu(hMenu, MF_POPUP, (UINT_PTR) hMenuSynPriority, TEXT("Change synthesizer thread priority"));
                        AppendMenu(hMenu, MF_SEPARATOR, 0, 0);
...
static void OutputString(const char *format, ...)
{
    ...
    va_start(va, format);
    vsnprintf(temp, ARRAY_SIZE(temp), format, va);
...
static void OutputWindowLastError(const char *message)
{
    ...
    temp = (char*) safe_large_malloc(strlen((char*) buffer) + strlen(message) + 10);
    sprintf(temp, "%s : %s" NLS, message, (char*) buffer);
...
static SERVICE_STATUS_HANDLE RegisterCtrlHandler(void)
{
