...
void InitListWnd(HWND hParentWnd)
{
    ...
    switch (PlayerLanguage) {
    case LANGUAGE_JAPANESE:
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_PLAY,"演奏(&P)");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDC_BUTTON_DOC,"ドキュメント(&D)...");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_SEPARATOR,0,0);
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_CURRENT,"現在位置(&C)");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_SEARCH,"検索(&S)...");
#ifdef LISTVIEW_PLAYLIST
        AppendMenuA(ListWndInfo.hPopupMenu,MF_SEPARATOR,0,0);
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_CUT,"カット(&X)");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_COPY,"コピー(&Z)");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_PASTE,"ペースト/挿入(&A)");
#endif
        AppendMenuA(ListWndInfo.hPopupMenu,MF_SEPARATOR,0,0);
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_REMOVE,"削除(&R)");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_SEPARATOR,0,0);
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_CHOOSEFONT,"フォントの選択(&H)...");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_LISTNAME,"リスト名変更(&L)...");
        break;
    case LANGUAGE_ENGLISH:
    default:
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_PLAY,"&Play");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDC_BUTTON_DOC,"&Doc...");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_SEPARATOR,0,0);
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_CURRENT,"&Current item");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_SEARCH,"&Search...");
#ifdef LISTVIEW_PLAYLIST
        AppendMenuA(ListWndInfo.hPopupMenu,MF_SEPARATOR,0,0);
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_CUT,"Cut(&X)");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_COPY,"Copy(&Z)");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_PASTE,"P&aste/Insert");
#endif
        AppendMenuA(ListWndInfo.hPopupMenu,MF_SEPARATOR,0,0);
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_REMOVE,"&Remove");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_SEPARATOR,0,0);
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_CHOOSEFONT,"C&hoose Font...");
        AppendMenuA(ListWndInfo.hPopupMenu,MF_STRING,IDM_LISTWND_LISTNAME,"Rename L&ist...");
        break;
    }
    hIcon = LoadImage(hInst, MAKEINTRESOURCE(IDI_ICON_TIMIDITY), IMAGE_ICON, 16, 16, 0);
...
ListWndProc(HWND hwnd, UINT uMess, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDC_BUTTON_CLEAR:
            if (MessageBoxA(hListWnd,"Clear playlist?","Playlist",
                            MB_YESNO)==IDYES)
#ifdef EXT_CONTROL_MAIN_THREAD
    ...
    case WM_VKEYTOITEM: {
        ...
        case 0x48:      // VK_H
            if ( PlayerLanguage == LANGUAGE_JAPANESE ) {
                MessageBoxA(hListWnd,
                "キーコマンド" NLS
                "リストウインドウコマンド" NLS
                "  ESC: ヘルプを閉じる      H: ヘルプを出す" NLS
                "  V: ドキュメントを見る      W: WRD ウインドウを開く" NLS
                "プレイヤーコマンド" NLS
                "  SPACE/ENTER: 演奏開始    E: 停止    S: 一時停止" NLS
                "  P: 前の曲    N: 次の曲" NLS
                "プレイリスト操作コマンド" NLS
                "  M: MIDIファイル以外を削除    U: 重複ファイルを削除" NLS
                "  C: プレイリストのクリア" NLS
#ifdef LISTVIEW_PLAYLIST
                "  Z: 選択した曲をコピー    X: 選択した曲をカット" NLS
                "  A: フォーカス位置へをペースト(挿入)" NLS
#endif
                "  D: カーソルの曲を削除    BS: カーソルの前の曲を削除" NLS
                "  INS: カーソルの曲をリストの最後に移す (Push)" NLS
                "  DEL: リストの最後の曲をカーソルの前に挿入 (Pop)" NLS
                "プレイリストタブ操作コマンド" NLS
                "  I: 次プレイリストタブ    O: 前プレイリストタブ" NLS
                "TiMidity コマンド" NLS
                "  Q: 終了" NLS
                ,"ヘルプ", MB_OK);
            } else {
                MessageBoxA(hListWnd,
                "Usage of key." NLS
                "List window command." NLS
                "  ESC: Close Help      H: Help" NLS
                "  V: View Document   W: Open WRD window" NLS
                "Player command." NLS
                "  SPACE/ENTER: PLAY    E: Stop    S: Pause" NLS
                "  P: Prev    N: Next" NLS
                "Playlist command." NLS
                "  M: Refine playlist    U: Uniq playlist" NLS
                "  C: Clear playlist" NLS
#ifdef LISTVIEW_PLAYLIST
                "  Z: Copy    X: Cut    A: Paste(Insert)" NLS
#endif
                "  D: Remove playlist    BS: Remove previous playlist" NLS
                "  INS: Push Playlist    DEL: Pop Playlist" NLS
                "Playlist tab command." NLS
                "  I: next playlist tab    O: prev playlist tab" NLS
...
void InitDocWnd(HWND hParentWnd)
{
    ...
    case LANGUAGE_JAPANESE:
        ...
        AppendMenuA(hMenu,MF_STRING,IDM_DOCWND_CHOOSEFONT,"フォントの選択");
        break;
    case LANGUAGE_ENGLISH:
    default:
        AppendMenuA(hMenu,MF_SEPARATOR,0,0);
        AppendMenuA(hMenu,MF_STRING,IDM_DOCWND_CHOOSEFONT,"Choose Font");
        break;
    }
    DocWndInfoReset2(hDocWnd);
    INILoadDocWnd();
...
SoundSpecWndProc(HWND hwnd, UINT uMess, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CHAR: {
        ...
        case 0x68:      // h
            if ( PlayerLanguage == LANGUAGE_JAPANESE ) {
                MessageBoxA(hSoundSpecWnd,
                "キーコマンド" NLS
                "スペクトログラムウインドウコマンド" NLS
                "  ESC: ヘルプを閉じる      H: ヘルプを出す" NLS
                "  UP: 縦ズームアウト    DOWN: 縦ズームイン" NLS
                "  LEFT: 横ズームアウト    RIGHT: 横ズームイン" NLS
                ,"ヘルプ", MB_OK);
            }
            else {
                MessageBoxA(hSoundSpecWnd,
                "Usage of key." NLS
                "Sound Spec window command." NLS
                "  ESC: Close Help      H: Help" NLS
                "  UP: Horizontal zoom out    DOWN: Horizontal zoom in" NLS
                "  LEFT: Vertical zoom out    RIGHT: Vertical zoom in" NLS
                ,"Help", MB_OK);
            }
            break;
        }
