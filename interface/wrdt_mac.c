...
#define CATTR_16FONT (1L<<1)
#define CATTR_COLORED (1L<<2)
#define CATTR_BGCOLORED (1L<<3)
#define CATTR_TXTCOL_MASK_SHIFT 4
#define CATTR_TXTCOL_MASK (7<<CATTR_TXTCOL_MASK_SHIFT)
#define CATTR_INVAL (1L<<31)
...
static const char *wrd_event2string(int id)
{
    const char *name;
...
static void wrdt_apply(int cmd, int wrd_argc, int wrd_args[])
{
    ...
    if (cmd == WRD_MAGPRELOAD) {
        //const char *p = wrd_event2string(arg);
        /* Load MAG file */
    ...
    case WRD_NL:
        if (cmd == WRD_NL)
            ...
            p = strdup_mblock(&tmpbuffer, wrd_event2string(wrd_args[0]));
            len = strlen(p);
