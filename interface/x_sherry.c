...
#define MAX_PLANES              8
#define MAX_COLORS              (1L<<MAX_PLANES)
...
static SherryPalette **virtualPalette; /* MAX_VIRTUAL_PALETTES, ²¾ÁÛ¥Ñ¥EÃ¥È */
...
static Bool init_palette(void)
{
    ...
    for (i = 0; i < MAX_COLORS; i++)
    {
        ...
        for (j = 0; j < MAX_PLANES; j++)
            if ((i & (1L << j)) != 0) palette2Pixel[i] |= planePixel[j];
    }
