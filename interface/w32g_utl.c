...
IniGetKeyInt32Array(char *section, char *key, int32 *n, int arraysize)
{
  ...
  for (i = 0; i < arraysize; i++) {
    snprintf(keybuffer, INI_MAXLEN - 1, "%s%d", key, i);
    GetPrivateProfileStringA
...
{
    CHAR buffer[INI_MAXLEN];
    GetPrivateProfileStringA(section, key, INI_INVALID, buffer,
...
{
  CHAR buffer[INI_MAXLEN] = { 0 };
  snprintf(buffer, INI_MAXLEN - 1, "%ld", *n);
  WritePrivateProfileStringA
    (section, key, buffer, IniFile);
  return 0;
}
...
{
  CHAR buffer[INI_MAXLEN] = { 0 };
  snprintf(buffer, INI_MAXLEN - 1, "%ld", (int)(*n));
  WritePrivateProfileStringA
    (section, key, buffer, IniFile);
  return 0;
}
...
IniPutKeyInt32Array(char *section, char *key, int32 *n, int arraysize)
{
  int i;
  CHAR buffer[INI_MAXLEN] = { 0 };
  CHAR keybuffer[INI_MAXLEN] = { 0 };
  for (i = 0; i < arraysize; i++) {
    snprintf(buffer, INI_MAXLEN - 1, "%ld", n[i]);
    snprintf(keybuffer, INI_MAXLEN - 1, "%s%d", key, i);
    WritePrivateProfileStringA(section, keybuffer, buffer, IniFile);
  }
  return 0;
}
...
{
  WritePrivateProfileStringA(section, key, str, IniFile);
  return 0;
}
...
{
    CHAR buffer[INI_MAXLEN];
    snprintf(buffer, INI_MAXLEN - 1, "%f", (double)n);
...
///r
    ...
    if (OutputName[0] && !is_device_output_ID(play_mode->id_character))
        play_mode->name = safe_strdup(OutputName);              // ÉÅÉÇÉäÉäÅ[ÉNÇ∑ÇÈÇ©Ç»ÅH ÇÕÇ∏Ç∑Ç∆ÇæÇﬂÇæÇµÅB
#elif 0
    if (OutputName[0] && !is_device_output_ID(play_mode->id_character))
...
void w32g_initialize(void)
{
    ...
    strcpy(SystemFont, "ÇlÇr ñæí©");
    strcpy(PlayerFont, "ÇlÇr ñæí©");
    strcpy(WrdFont, "ÇlÇr ñæí©");
    strcpy(DocFont, "ÇlÇr ñæí©");
    strcpy(ListFont, "ÇlÇr ñæí©");
    strcpy(TracerFont, "ÇlÇr ñæí©");
    ...
    else
    {
        const char *confirm,
            confirm_en[] = "Ini file is not found, or old format is found." NLS
                           ...
                           "Ini file path: %s",
            confirm_ja[] = "ê›íËÉtÉ@ÉCÉãÇ™å©Ç¬Ç©ÇÁÇ»Ç¢Ç©å√Ç¢å`éÆÇ≈Ç∑ÅB" NLS
                           "ê›íËÉtÉ@ÉCÉãÇèâä˙âªÇµÇƒÇÊÇÎÇµÇ¢Ç≈Ç∑Ç©ÅH" NLS NLS
                           "ê›íËÉtÉ@ÉCÉãÇÃÉpÉX: %s";
...
#else
    if (GetModuleFileNameA(hInst, buffer, FILEPATH_MAX - inilen - 1))
    {
...
#endif
        dwRes = GetFileAttributesA(timidity_output_inifile);
    ...
    else
    {
        const char *confirm,
            confirm_en[] = "Ini file is not found, or old format is found." NLS
                           ...
                           "Ini file path: %.1023s",
            confirm_ja[] = "ê›íËÉtÉ@ÉCÉãÇ™å©Ç¬Ç©ÇÁÇ»Ç¢Ç©å√Ç¢å`éÆÇ≈Ç∑ÅB" NLS
                           "ê›íËÉtÉ@ÉCÉãÇèâä˙âªÇµÇ‹Ç∑Ç©ÅH" NLS NLS
                           "ê›íËÉtÉ@ÉCÉãÇÃÉpÉX: %.1023s";
        switch (PlayerLanguage) {
        case LANGUAGE_ENGLISH: confirm = confirm_en; break;
        case LANGUAGE_JAPANESE: default: confirm = confirm_ja; break;
        }
