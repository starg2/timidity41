...
static void init_lyric(const char *lang);
...
static void ctl_head(void)
{
  SLsmg_printfrc(0, 0, "TiMidity++ %s%s",
                (!strstr(timidity_version, "current")) ? "v" : "", timidity_version);
...
static int32 ctl_write(const uint8 *valp, int32 size)
{
  static int warned = 0;
  if (!warned) {
    fprintf(stderr, "Warning: using stdout with slang interface will not" NLS
