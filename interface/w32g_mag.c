...
int pho_load_pixel(char *image, int width, int height, const char *filename)
{
        int i,j,k,l;
                ...
                for (i=0;i<400;i++) {
                        ...
                        for (j=0;j<sizeof(buffer);j++) {
                                for (k=0;k<8;k++) {
                                        ...
                                        if ( ( buffer[j]<<k ) & 0x80 )
                                                image[ j * 8 + k + i * width ] = image[ j * 8 + k + i * width] | ( 1L<< shift[l] );
                                        else
...
magdata *mag_search(const char *filename)
{
        magdata *m;
...
magdata *mag_create(const char *file)
{
...
error:
        ...
        {
                close_file(fp);
                ctl->cmsg(CMSG_INFO,VERB_VERBOSE,"Mag error: %s", file);
        }
        free(mg.filename);
        safe_free(flaga);
        safe_free(flagb);
