...
static uint32 color_ring[NCOLOR];
...
static int32 search_near_color(Display *disp, int r, int g, int b)
{
    double d, mind;
    int scr;
    ...
    if (depth == 1)             /* black or white */
    {
        ...
        if (d > 3.0 * 32768.0 * 32768.0)
            return (int32) WhitePixel(disp, scr);
        return (int32) BlackPixel(disp, scr);
    }
...
void soundspec_update_wave(DATA_T *buff, int samples)
{
    ...
    if (buff == NULL) /* Initialize */
    {
        ...
        if (ring_buffer != NULL)
            memset(ring_buffer, 0, ring_buffer_len * sizeof(DATA_T));
        return;
    }
    ...
    if (ring_buffer == NULL)
    {
        ring_buffer = (DATA_T*) safe_malloc(ring_buffer_len * sizeof(DATA_T));
        memset(ring_buffer, 0, ring_buffer_len * sizeof(DATA_T));
        if (soundspec_update_interval == 0)
            soundspec_update_interval =
...
#endif /* X Window System platforms */
