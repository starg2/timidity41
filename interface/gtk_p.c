...
pipe_error(char *st)
{
    fprintf(stderr,"CONNECTION PROBLEM WITH Gtk+ PROCESS IN %s BECAUSE:%s" NLS,
            st,
            strerror(errno));
    exit(1);
}
...
gtk_pipe_int_read(int *c)
{
    ...
    if (code!=INT_CODE)
        fprintf(stderr,"BUG ALERT ON INT PIPE %i" NLS,code);
#endif
...
gtk_pipe_string_read(char *str)
{
    ...
    if (code!=STRING_CODE) fprintf(stderr,"BUG ALERT ON STRING PIPE %i" NLS,code);
#endif
...
gtk_pipe_open(void)
{
    ...
    if ((pid=fork())==0) {   /*child*/
        ...
        fprintf(stderr,"WARNING: come back from Gtk+" NLS);
        exit(0);
    }
