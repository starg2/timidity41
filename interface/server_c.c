...
static int32 ctl_write(const uint8 *buffer, int32 size);
...
static int32 ctl_write(const uint8 *buffer, int32 size)
{
    ...
    if (data_fd != -1)
#ifdef __W32__
        return send(data_fd, buffer, size, 0);
#else
...
static int pasv_open(int *port)
{
    ...
    for (rp = result; rp != NULL; rp = rp->ai_next)
    {
...
#ifdef SO_REUSEADDR
        {
            int on = 1;
#ifdef __W32__
            setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, (LPCSTR) &on, sizeof(on));
#else
            ...
            switch (data[4])
            {
              ...
              case 0x02:  { /* Used to sync. */
                    ...
                    if (sleep_time > 0)
                    {
                        ...
                        usleep((uint32)(sleep_time * 1000000));
                    }
                    else
                        send_status(301, "0 Sync OK");
                }
      ...
      case SEQ_CONTROLLER:
        switch (data[4])
        {
          ...
          case CTRL_PITCH_BENDER_RANGE: /* ?? */
            ev.channel = MIDIPORT_MASK(data[3]);
            ev.b = 0;
            ev.a = 0;
