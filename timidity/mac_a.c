...
#ifdef SOUND_MANAGER_3_OR_LATER
					...
					SndCallBackUPP userRoutine,	short	queueLength)
{
	...
	if (mySndChan != 0) {
		...
		if (myErr != noErr) {			// {couldn't allocate channel}
			safe_free(mySndChan); // {free memory already allocated}
...
static void CleanupDriverMemory(void)
{
	if (gSndCannel)
	{
		...
		safe_free(gSndCannel);
		gSndCannel = NULL;
	}
	safe_free(soundBuffer);
	soundBuffer = NULL;
	safe_free(soundHeader);
	soundHeader = NULL;
	safe_free(soundBufferMasterPtr);
...
static int open_output(void)
{
	...
	for (i = 0; i < sndBufferCount; i++)
	{
		...
		#ifdef SOUND_MANAGER_3_OR_LATER	// supports fixedCompression
		header->futureUse2 = 0;
