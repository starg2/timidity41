...
static UWORD 
getlogperiod (UWORD note, ULONG fine)
{
  UWORD n, o;
  UWORD p1, p2;
  ULONG i;
...
static UWORD 
getoldperiod (UWORD note, ULONG speed)
{
  UWORD n, o;
...
static UWORD 
GetPeriod (UWORD note, ULONG speed)
{
	  ...
	  if (p >= t->env[b].pos)
	      ...
	      else
		{
		  if (b >= t->pts)
			...
			{
			  mp.voice[mp_channel].keyoff |= KEY_FADE;
			  if (!v)
			    mp.voice[mp_channel].fadevol = 0;
			}
		      b--;
		      p--;
		    }
...
ULONG 
getfrequency (UBYTE flags, ULONG period)
{
  if (flags & UF_LINEAR)
...
pt_UpdateVoices ()
{
  ...
  for (mp_channel = 0; mp_channel < MOD_NUM_VOICES; mp_channel++)
    {
      aout = &mp.voice[mp_channel];
      i = aout->i;
      s = aout->s;
	...
	ProcessEnvelope (&aout->cenv, 32, aout->keyoff);
      ...
      Voice_SetVolume (mp_channel, tmpvol);
      if ((tmpvol) && (aout->master) && (aout->master->slave == aout))
	mp.realchn++;
      mp.totalchn++;

      if (aout->panning == PAN_SURROUND)
	Voice_SetPanning (mp_channel, PAN_SURROUND);
      else if (aout->penv.flg & EF_ON)
	Voice_SetPanning (mp_channel, DoPan (envpan, aout->panning));
      else
	Voice_SetPanning (mp_channel, aout->panning);
	...
	{
	  Voice_SetFrequency (mp_channel,
			      getfrequency (pf->flags, playperiod));

	  if (kick_voice)
	    Voice_Play (mp_channel, s, (aout->start == -1) ? ((s->flags & SF_UST_LOOP) ? s->loopstart : 0) : aout->start);
...
pt_Notes (void)
{
  ...
  for (mp_channel = 0; mp_channel < pf->numchn; mp_channel++)
	...
	{
	  tr = pf->patterns[(pf->positions[mp.sngpos] * pf->numchn) + mp_channel];
	  ...
	  if (a->dct != DCT_OFF)
	    {
	      ...
	      for (t = 0; t < MOD_NUM_VOICES; t++)
		if ((!Voice_Stopped (t)) &&
		    (mp.voice[t].masterchn == mp_channel) &&
  ...
  for (mp_channel = 0; mp_channel < pf->numchn; mp_channel++)
      ...
      if (a->kick == KICK_NOTE)
	{
	  ...
	  if (pf->flags & UF_NNA)
	    ...
	    a->slave = &mp.voice[a->slavechn = mp_channel];
...
HandleTick (void)
{
  ...
  if (++mp.vbtick >= mp.sngspd)
      ...
      if (mp.posjmp)
	{
	  ...
	  for (mp_channel = 0; mp_channel < pf->numchn; mp_channel++)
	    mp.control[mp_channel].pat_reppos = -1;
