...
int load_vorbisenc_dll(void)
{
	if (!h_vorbisenc_dll) {
		...
		h_vorbisenc_dll = LoadLibrary(TEXT("vorbisenc.dll"));
		if (!h_vorbisenc_dll) h_vorbisenc_dll = LoadLibrary(TEXT("libvorbisenc-2.dll"));
		if (!h_vorbisenc_dll) h_vorbisenc_dll = LoadLibrary(TEXT("libvorbisenc.dll"));
...
/***************************************************************/
#endif /* AU_VORBIS_DLL */
#endif /* ! VORBIS_DLL_INCLUDE_VORBISENC */