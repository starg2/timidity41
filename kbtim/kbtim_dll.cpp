...
static HINSTANCE g_hDll = NULL; //kbtim.dll のインスタンスハンドル
static LONG g_lCanUse = -1; //InterlocedIncrement(&g_lCanUse) == 0 のとき使用可能
static const int UNIT_SAMPLE = 1024;
static const int PRELOAD_MS = 3500; //通常時
static const int PRELOAD_MS_REALTIME_PLAYING = 5500;//演奏中に音色読込有効時
    //先読みバッファのサイズ（ミリ秒）
    //値が大きいほど、
    //・音とびしにくくなる
    //・メモリ使用量が大きくなる
                                   ...
                                   SOUNDINFO *pInfo)
{
    ...
    SetThreadPriority(m_hThread, THREAD_PRIORITY_NORMAL);
    //再生周波数要求
    //384, 352.8, 256, 192, 176.4, 128, 96, 88.2, 64, 48, 44.1, 32, 22.5 kHz のみ要求に応じる
...
DWORD __fastcall KbTimDecoder::Render(BYTE *pBuffer, DWORD dwSize)
{
    ...
    if(m_nStart == 1 && !m_bEOF){
        ...
        //初回 Render 時のみ、先読みバッファ分が全てデコードされるまで待つ
            if(!m_bEOF){
                LeaveCriticalSection(&m_cs);
                Sleep(1);
                EnterCriticalSection(&m_cs);
...
{//実質的に使用不可能
 //2回目以降に kbtim_GetModule が呼ばれたら返す
    KMPMODULE_VERSION,
    KMPMODULE_PLUGIN_VERSION,
    g_cszKbTimCopyright,
    g_cszKbTimDescription,
...
////////////////////////////////////////////////////////////////////////
//kbtim.dll は１つの kbtim(_xxx).kpi からしか利用できない
//1回目は KMPMODULE::dwReentrant == 0 で利用可能な KMPMODULE(g_Module) を返す
//2回目以降は KMPMODULE::dwReentrant == 0xFFFFFFFF にセットして実質的に利用不可能な 
//KMPMODULE(g_NullModule) を返し、KbMedia Player からは常に kbrunkpi.exe 経由で利用することを促す
KMPMODULE* WINAPI kbtim_GetModule(HINSTANCE hInstance, DWORD dwVersion)
{
    if((dwVersion == KBTIM_GETMODULE_VERSION) && hInstance){
        ...
        if(g_hKpi == hInstance){//初めて呼ばれたか、同じ kpi によって呼ばれた場合
            pRet = &g_Module;
        }
        else{
            pRet = &g_NullModule;
