...
{
    if(dwVersion == 0){
        //プラグインと同じフォルダにある kbtimsetup.exe を起動する
        //コマンドラインパラメータとして、自分自身(kbtim(_xxx).kpi) のフルパスを渡す
...
static void load_wrapper_config(void)
{
	...
	if(!kpi_wrapper){	// in_kbtim
		...
		memcpy(&Module, pModule, sizeof(KMPMODULE));
		if(pModule->dwReentrant != 0xFFFFFFFF) //kbtim.dll が未使用
			g_pModule = pModule;
		pModule = &Module;
		g_hDll2 = hDll;    
	}else{ // in_kpi_wrapper
...
{ 	
	...
	if(!hkmp)
		return 1;
	if(InfoKPI.dwUnitRender <= 0){ // なぜか0にされる場合
