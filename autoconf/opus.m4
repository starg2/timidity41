...
dnl
AC_ARG_WITH(opus,
	    AS_HELP_STRING([--with-opus=PFX],
	    		   [Prefix where libopus is installed (optional)]),
	    [opus_prefix="$withval"], [opus_prefix=""])
AC_ARG_WITH(opus-libraries,
	    AS_HELP_STRING([--with-opus-libraries=DIR],
			   [Directory where libopus library is installed (optional)]),
	    [opus_libraries="$withval"], [opus_libraries=""])
AC_ARG_WITH(opus-includes,
	    AS_HELP_STRING([--with-opus-includes=DIR],
	    		   [Directory where libopus header files are installed (optional)]),
	    [opus_includes="$withval"], [opus_includes=""])
AC_ARG_ENABLE(opustest,
	      AS_HELP_STRING([--disable-opustest],
			     [Do not try to compile and run a test Opus program]),
	      , [enable_opustest=yes])
  ...
  OPUS_LIBS="$OPUS_LIBS -lopus -lm"
  OPUSFILE_LIBS="-lopusfile"
  OPUSENC_LIBS="-lopusenc"
  ...
  if test "x$enable_opustest" = "xyes" ; then
    ...
    ac_save_LIBS="$LIBS"
    CFLAGS="$CFLAGS $OPUS_CFLAGS $OGG_CFLAGS"
    LIBS="$LIBS $OPUS_LIBS $OPUSENC_LIBS $OGG_LIBS"
dnl
